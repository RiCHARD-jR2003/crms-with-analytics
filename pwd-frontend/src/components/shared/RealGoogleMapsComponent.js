...
import { generateMapsEmbedUrl, generateMapsSearchUrl, isApiKeyConfigured } from '../../config/googleMapsConfig';
...
const BARANGAYS = [
  ...
  { name: 'Poblacion Dos', lat: 14.2500, lng: 121.1250, color: '#FF5722' },
  { name: 'Poblacion Tres', lat: 14.2520, lng: 121.1260, color: '#CDDC39' },
  { name: 'Banaybanay', lat: 14.2200, lng: 121.1100, color: '#FFEB3B' },
  { name: 'Baclaran', lat: 14.1900, lng: 121.1400, color: '#00E676' }
];

const RealGoogleMapsComponent = ({ onBarangaySelect, height = '400px' }) => {
  ...
  const openInGoogleMaps = () => {
    const url = generateMapsSearchUrl(PWD_OFFICE_LOCATION.address);
    window.open(url, '_blank');
  };

  const openBarangayInMaps = (barangay) => {
    const url = generateMapsSearchUrl(barangay.name + ', Cabuyao City, Laguna');
    window.open(url, '_blank');
  };
  ...
  const mapsEmbedUrl = generateMapsEmbedUrl(mapCenter.lat, mapCenter.lng);

  // Set map as loaded initially since we're using embed
  useEffect(() => {
    setMapLoaded(true);
  }, []);
      ...
      {!isApiKeyConfigured() && (
        <Alert severity="info" sx={{ mb: 2 }}>
          <Typography variant="body2">
            <strong>Note:</strong> Using public Google Maps embed. For enhanced features, 
            <Button 
              size="small" 
      ...
      <Box sx={{ 
        ...
        {mapError ? (
          ...
          <Box sx={{ 
            ...
            <Alert severity="warning" sx={{ width: '100%', maxWidth: '400px', mb: 2 }}>
              <Typography variant="h6" sx={{ mb: 1 }}>
                Google Maps Not Available
              </Typography>
              <Typography variant="body2" sx={{ mb: 2 }}>
                Using alternative map view. Click below to open in Google Maps.
              </Typography>
              <Button
                variant="contained"
                startIcon={<OpenInNew />}
          ...
          <Box sx={{ position: 'relative', height: '100%' }}>
            ...
            <iframe
              ...
              title="Google Maps - Cabuyao PWD Office"
...
export default RealGoogleMapsComponent;
